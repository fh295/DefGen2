...
def gen_sample(tparams, f_init, f_next, ctx, options, trng=None, k=1, maxlen=30):
    ...
    # sample best k first words
    ctx = numpy.tile(ctx0, [k, 1]) 
    next_p, next_w, next_state, next_memory = f_next(next_w, ctx, next_state, next_memory)
    init_scores = - numpy.log(next_p[0])
    best_k_init = init_scores.argsort()[:k]
    live_scores = init_scores[best_k_init]
    live_samples = best_k_init[:,None]
    next_w = numpy.array([w[-1] for w in live_samples])
    
    for __ in xrange(1, maxlen):
        ctx = numpy.tile(ctx0, [live_k, 1]) 
        # now, next_p and next_w , next_state, next_memory contain live_k hypotheses each
        next_p, next_w, next_state, next_memory = f_next(next_w, ctx, next_state, next_memory)
        cand_scores = live_scores - numpy.log(next_p) 
        # flatten to compare all words over all hypothesis branches
        cand_flat = cand_scores.flatten() 
        # select live-k indices with the lowest -logprobs 
        ranks_flat = cand_flat.argsort()[:live_k] 
        
        voc_size = next_p.shape[1] 
        # translate indices in ranks_flat into previous hypothesis that score came from
        trans_indices = ranks_flat / voc_size  
        # and within each hypothesis, to the new word that score corresponds
        word_indices = ranks_flat % voc_size
        costs = cand_flat[ranks_flat]   # the live_k best costs selected in this case
        ...
        for idx, [ti,wi] in enumerate(zip(trans_indices, word_indices)):
            new_samples.append(live_samples[ti]+wi)
            new_scores.append(copy.copy(costs[idx]))
            new_states.append(copy.copy(next_state[ti])) 
            new_memories.append(copy.copy(next_memory[ti]))
        ...
        for idx in xrange(len(new_samples)):
            # check if any samples have terminated \
            # if so, add sample and score to final output
            if new_samples[idx][-1] == 0:  
                sample.append(new_samples[idx])  
                sample_score.append(new_scores[idx])  
                dead_k += 1  
                live_k -= 1  # now looking for fewer samples
            else: 
                live_samples.append(copy.copy(new_samples[idx]))  
                live_scores.append(copy.copy(new_scores[idx])) 
                live_states.append(copy.copy(new_states[idx])) 
                live_memories.append(copy.copy(new_memories[idx])) 
        next_w = numpy.array([w[-1] for w in live_samples])
        old_scores = numpy.array(live_scores)
        next_state = numpy.array(live_states)
        next_memory = numpy.array(live_memories)
    ...
    # after max-length is reached, dump out samples that failed to terminate
    if live_k > 0:
